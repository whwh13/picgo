...
struct VertexData
    ...
    species_count :: SVector{4, Float64}
end
            ...
            if splitted[1] == "FRAME"
                ...
                push!(
                    ...
                    VertexData(
                        ...
                        SVector{4}([parse(Int, splitted[6]), parse(Int, splitted[7]), parse(Int, splitted[8]), parse(Int, splitted[9])])
                    )
                )
            end
        end
...
begin
    ...
    expected_uniform_a = [max_copy_a * v.cell_area / total_area for v ∈ frames[1].vertices]
	expected_uniform_b = [max_copy_b * v.cell_area / total_area for v ∈ frames[1].vertices]
    expected_sin = [v.cell_area * exp(-β * energy_at_coord(v.coord)) for v ∈ frames[1].vertices]
    expected_sin_diffu = expected_sin * (max_copy_diffu / sum(expected_sin))
	expected_sin_ad    = expected_sin * (max_copy_ad    / sum(expected_sin)) * 0.8  # Fit manually, accounting for unbound bulk species.
	
	expected_conc = [expected_uniform_a, expected_uniform_b, expected_sin_diffu, expected_sin_ad]
end

# ╔═╡ 2e4b60fd-e59c-42e0-b68e-9da50f73c09c

function make_avg_plot(species_index)
...
md"
Species index $(@bind species_index Slider(1:4; show_value=true))
"

# ╔═╡ 5b4da84b-f765-4e49-9251-0c2426985ff0
begin
	avg_plots = [make_avg_plot(species_index) for species_index ∈ 1:4]
	avg_plots[species_index]
end

# ╔═╡ f039239f-f54a-4b62-9205-660135415db5
begin
	savefig(avg_plots[3], joinpath(work_dir, "surface-mesh-diffusion-avg.png"))
	savefig(avg_plots[4], joinpath(work_dir, "surface-mesh-adsorption-avg.png"))
end

# ╔═╡ 61623a45-1616-44d0-b468-0658a13fb49c
function make_anim(species_index)
...
# ╔═╡ e690b259-c638-4f35-aa76-060f036200d5
anims = [make_anim(species_index) for species_index ∈ 1:4]

# ╔═╡ 22885733-253a-4836-bbed-3a645f7637d9
gif(anims[3], joinpath(work_dir, "surface-mesh-diffusion-anim.gif"); fps=4)

# ╔═╡ 14419212-5476-4e7e-9c02-dcbd4f342102
gif(anims[4], joinpath(work_dir, "surface-mesh-adsorption-anim.gif"); fps=4)
