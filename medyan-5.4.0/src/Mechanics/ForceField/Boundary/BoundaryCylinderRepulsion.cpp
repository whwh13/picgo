...
void BoundaryCylinderRepulsion<BRepulsionInteractionType>::vectorize(const FFCoordinateStartingIndex& si, const SimulConfig& conf) {
...
template< typename InteractionType >
void BoundaryCylinderRepulsion< InteractionType >::computeLoadForce(SubSystem& sys, Cylinder* c, LoadForceEnd end) const {
    for (auto be : BoundaryElement::getBoundaryElements()) {
